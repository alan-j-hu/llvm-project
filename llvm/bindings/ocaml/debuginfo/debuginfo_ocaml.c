...
value llvm_debug_metadata_version(value Unit) {
  CAMLparam1(Unit);
  CAMLreturn(Val_int(LLVMDebugMetadataVersion()));
}

value llvm_get_module_debug_metadata_version(value Module) {
  CAMLparam1(Module);
  CAMLreturn(Val_int(LLVMGetModuleDebugMetadataVersion(Module_val(Module))));
}
...
value llvm_dibuild_create_function_native(value Builder, value Scope,
  ...
  CAMLparam5(Builder, Scope, Name, LinkageName, File);
  CAMLxparam5(LineNo, Ty, IsLocalToUnit, IsDefinition, ScopeLine);
  CAMLxparam2(Flags, IsOptimized);
...
value llvm_metadata_null(value Unit) {
  CAMLparam1(Unit);
...
value llvm_dibuild_create_debug_location(value Ctx, value Line, value Column,
  ...
  CAMLreturn(to_val(LLVMDIBuilderCreateDebugLocation(
...
value llvm_dibuild_create_union_type_native(
  ...
  CAMLparam5(Builder, Scope, Name, File, LineNumber);
  CAMLxparam5(SizeInBits, AlignInBits, Flags, Elements, RunTimeLanguage);
...
value llvm_dibuild_create_struct_type_native(
  ...
  CAMLxparam3(RunTimeLanguage, VTableHolder, UniqueId);
  LLVMMetadataRef *Temp = alloc_temp(Elements);
...
value llvm_dibuild_create_member_pointer_type_native(
  ...
  CAMLparam5(Builder, PointeeType, ClassType, SizeInBits, AlignInBits);
  CAMLxparam1(Flags);
