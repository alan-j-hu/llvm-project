...
value llvm_datalayout_preferred_align_of_global(value GlobalVar, value DL) {
  ...
  CAMLreturn(Val_int(LLVMPreferredAlignmentOfGlobal(DataLayout_val(DL),
                                                    Value_val(GlobalVar))));
}
...
value llvm_datalayout_element_at_offset(value Ty, value Offset, value DL) {
  ...
  CAMLreturn(Val_int(LLVMElementAtOffset(DataLayout_val(DL), Type_val(Ty),
                                         Int64_val(Offset))));
}
